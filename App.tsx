...
import { processInventoryData, aggregateSedeMetrics, getTrafficLightColor, getStatusLabel } from './utils/calculations';
...
const App: React.FC = () => {
  ...
  const [consecutive, setConsecutive] = useState<string>('Cobro000');

  useEffect(() => {
    const savedCounter = localStorage.getItem('reliability_pro_consecutive_counter');
    const currentCounter = savedCounter ? parseInt(savedCounter) : 1;
  ...
  const handleDataLoaded = (raw: InventoryRawRow[]) => {
    const savedCounter = localStorage.getItem('reliability_pro_consecutive_counter');
    const nextCounter = savedCounter ? parseInt(savedCounter) + 1 : 1;
    localStorage.setItem('reliability_pro_consecutive_counter', nextCounter.toString());
    setConsecutive(`Cobro${nextCounter.toString().padStart(3, '0')}`);
  ...
  const handleDownloadReport = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      const canvas = await html2canvas(el, { scale: 2, useCORS: true, backgroundColor: "#ffffff", windowWidth: 1240, logging: false });
      ...
      while (heightLeft > 0) { position -= pageHeight; pdf.addPage(); pdf.addImage(imgData, "PNG", 0, position, imgWidth, imgHeight); heightLeft -= pageHeight; }
      const dateStr = new Date().toISOString().slice(0, 10).replace(/-/g, '_');
      const sedeFriendly = selectedSede === 'Todas' ? 'General' : selectedSede.replace(/\s+/g, '_');
      const ccFriendly = selectedCentroCosto === 'Todos' ? 'MultiCC' : selectedCentroCosto.replace(/\s+/g, '_');
      pdf.save(`${consecutive}_${sedeFriendly}_${ccFriendly}_${dateStr}.pdf`);
    } catch (err) { console.error("Error en exportación PDF:", err); } finally { setIsDownloading(false); }
  };

  const formatCurrency = (val: number) => {
    const formatted = new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 }).format(Math.abs(val));
    return val < 0 ? `-${formatted}` : formatted;
  };

  const renderPrintView = () => (
    <div id="PDF_EXPORT_CONTAINER" className="bg-white p-12 text-slate-900" style={{ width: '1240px', minHeight: '1600px', position: 'fixed', top: 0, left: 0, zIndex: 999999, overflow: 'visible', backgroundColor: '#ffffff' }}>
      <div className="flex justify-between items-center mb-16 border-b-8 border-slate-900 pb-12">
        <div className="flex items-center gap-10">
          <div className="bg-emerald-600 p-8 rounded-[40px] text-white shadow-2xl"><i className="fa-solid fa-brain text-7xl"></i></div>
          <div>
            <h1 className="text-6xl font-black uppercase tracking-tighter leading-none mb-3">RELIABILITY PRO: {activeView === 'dashboard' ? 'RESUMEN EJECUTIVO' : 'REPORTE DE AUDITORÍA'}</h1>
            <div className="flex gap-6 items-center">
              <p className="text-emerald-600 font-black uppercase tracking-[0.4em] text-xl">Soporte Operativo Certificado</p>
              <div className="bg-slate-100 px-4 py-1 rounded-lg"><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mr-2">ID Reporte:</span><span className="text-sm font-black text-slate-700">{consecutive}</span></div>
            </div>
          </div>
        </div>
        <div className="text-right border-l-4 border-slate-100 pl-10"><p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Fecha Emisión:</p><p className="font-black text-slate-800 text-3xl">{new Date().toLocaleDateString('es-CO')}</p></div>
      </div>
        ...
        {activeView === 'dashboard' ? (
          <div className="space-y-16">
            <div className="grid grid-cols-3 gap-8">
               <div className="bg-white p-10 border-2 border-slate-100 rounded-[40px] text-center"><p className="text-xs font-black text-slate-400 uppercase mb-4">Salud Global</p><p className="text-6xl font-black text-slate-900">{(filteredData.filter(i=>i.reliability===1).length / (filteredData.length || 1) * 100).toFixed(1)}%</p></div>
               <div className="bg-rose-50 p-10 border-2 border-rose-200 rounded-[40px] text-center"><p className="text-xs font-black text-rose-400 uppercase mb-4">Monto Liquidación</p><p className="text-5xl font-black text-rose-600">{formatCurrency(filteredData.reduce((a,b)=>a+(b.Estado_Normalizado==='Faltantes'?-b.Cobro:b.Cobro),0))}</p></div>
               <div className="bg-slate-900 p-10 rounded-[40px] text-center text-white"><p className="text-xs font-black text-slate-500 uppercase mb-4">Total Ajustes</p><p className="text-5xl font-black">{formatCurrency(filteredData.reduce((a,b)=>a+b["Costo Ajuste"], 0))}</p></div>
            </div>
            <table className="w-full border-collapse">
              <thead><tr className="bg-slate-900 text-white uppercase text-[14px] font-black tracking-widest"><th className="p-8 text-left">Almacén Auditado</th><th className="p-8 text-center">CONFIABILIDAD DE INVENTARIOS</th><th className="p-8 text-right">Impacto Balance</th></tr></thead>
              <tbody>{sedeMetrics.map(m => (
                <tr key={m.almacen} className="border-b-2 border-slate-50"><td className="p-8 font-black uppercase text-slate-900 text-2xl">{m.almacen}</td><td className="p-8 text-center"><span className="font-black text-2xl">{m.globalReliability.toFixed(1)}%</span><span className="ml-3 text-xs font-black text-slate-400">({getStatusLabel(m.globalReliability)})</span></td><td className={`p-8 text-right font-black text-2xl ${m.totalCobro < 0 ? 'text-rose-600' : 'text-emerald-600'}`}>{formatCurrency(m.totalCobro)}</td></tr>
              ))}</tbody>
            </table>
          </div>
        ) : (
          <div className="space-y-12">
            <div className="p-10 bg-slate-50 rounded-[40px] border-2 border-slate-100">
               ...
               <div className="grid grid-cols-3 gap-8">
                  {ccReliabilityReport.map(cc => (
                    <div key={cc.name} className="bg-white p-6 rounded-3xl border border-slate-200 flex justify-between items-center shadow-sm">
                       <div><p className="text-[10px] font-black text-slate-400 uppercase mb-1">{cc.name}</p><p className="text-3xl font-black text-slate-800">{cc.reliability.toFixed(1)}%</p></div>
                       <div className={`px-4 py-2 rounded-xl bg-${getTrafficLightColor(cc.reliability)}-50 text-${getTrafficLightColor(cc.reliability)}-600 font-black text-sm`}>{getStatusLabel(cc.reliability)}</div>
                    </div>
                  ))}
               </div>
            </div>
            ...
            <table className="w-full border-collapse">
              <thead><tr className="bg-slate-900 text-white uppercase text-[12px] font-black tracking-widest"><th className="p-6 text-left">Artículo / Centro Costo</th><th className="p-6 text-center">Unidad</th><th className="p-6 text-center">Variación</th><th className="p-6 text-right bg-slate-800">Valor Neto</th></tr></thead>
              <tbody>{filteredData.map(item => {
                ...
                return (
                  <tr key={item.id} className="border-b border-slate-100"><td className="p-6"><p className="font-black uppercase text-xl text-slate-800 mb-1">{item.Artículo}</p><p className="text-xs text-slate-400 font-black uppercase tracking-widest">{item["Centro de Costos"]} — {item.Estado_Normalizado}</p></td><td className="p-6 text-center font-bold text-slate-400 text-lg">{item.Unidad}</td><td className={`p-6 text-center font-black text-2xl ${isFaltante ? 'text-rose-600' : 'text-emerald-600'}`}>{item["Variación Stock"]}</td><td className={`p-6 text-right font-black text-2xl ${isFaltante ? 'text-rose-600' : 'text-emerald-600'}`}>{item.Estado_Normalizado !== 'Sin Novedad' ? formatCurrency(displayVal) : '0'}</td></tr>
                );
              })}</tbody>
              <tfoot><tr className="bg-slate-50"><td colSpan={3} className="p-12 text-right font-black uppercase text-xl tracking-widest text-slate-400">Balance Final Reportado:</td><td className={`p-12 text-right font-black text-6xl border-t-[10px] ${filteredData.reduce((a,b)=>a+(b.Estado_Normalizado==='Faltantes'?-b.Cobro:b.Cobro),0) < 0 ? 'text-rose-600 border-rose-600' : 'text-emerald-600 border-emerald-600'}`}>{formatCurrency(filteredData.reduce((a,b)=>a+(b.Estado_Normalizado==='Faltantes'?-b.Cobro:b.Cobro),0))}</td></tr></tfoot>
            </table>
          </div>
        )}
      </div>
      ...
      <div className="mt-32 pt-24 border-t-8 border-slate-900">
         <div className="flex justify-between gap-40 mb-24">
            <div className="flex-1 border-t-2 border-slate-100 pt-6"><p className="text-sm font-black uppercase text-slate-400 mb-16">Firma Auditoría Senior</p><div className="h-0.5 bg-slate-50 w-full mb-3"></div><p className="text-xs font-bold text-slate-300 uppercase">Validación de Datos</p></div>
            <div className="flex-1 border-t-2 border-slate-100 pt-6"><p className="text-sm font-black uppercase text-slate-400 mb-16">Firma Gerente General</p><div className="h-0.5 bg-slate-50 w-full mb-3"></div><p className="text-xs font-bold text-slate-300 uppercase">Autorización Administrativa</p></div>
         </div>
        ...
        <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-10">
          {data.length === 0 ? (<div className="flex flex-col items-center justify-center h-[60vh] text-slate-400"><i className="fa-solid fa-file-excel text-8xl mb-6 text-emerald-600 opacity-20"></i><h2 className="text-2xl font-black uppercase tracking-tighter">Sin Datos Cargados</h2><p className="text-sm font-medium">Use el botón superior para importar el reporte de Excel.</p></div>) : (
            <div>
              ...
              {activeView === 'cobros' && <CobrosReport data={filteredData} selectedSede={selectedSede} selectedCentroCosto={selectedCentroCosto} selectedEstado={selectedEstado} startDate={startDate} endDate={endDate} consecutive={consecutive} />}
              {activeView === 'critical' && <CriticalItems data={filteredData} />}
            </div>
          )}
        </main>
        <footer className="py-10 border-t border-slate-200 text-center no-print"><p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Reliability Pro &copy; {new Date().getFullYear()} - Auditoría de Restaurantes</p></footer>
      </div>
    </div>
  );
};
...
const NavBtn = ({ active, label, onClick, highlight = false }: any) => (
  <button onClick={onClick} className={`px-8 py-5 text-[11px] font-black uppercase tracking-widest border-b-4 transition-all whitespace-nowrap ${active ? 'border-emerald-600 text-emerald-600' : highlight ? 'text-emerald-600/60' : 'text-slate-400 hover:text-slate-600'}`}>{label}</button>
);
